...
public class InsightARManager  : MonoBehaviour
{
    ...
    void OnEnable()
    {
        InsightARPluginFunctions.InsightARFrameUpdatedEvent += FrameUpdate;
        InsightARPluginFunctions.RegisterAppKeyI("AppKey", "AppSecret");
        InsightARPluginFunctions.InitAR(algPath);
    ...
    private void DoStartAR()
    {
        ...
        {
            #if UNITY_IOS
            _algPath = Application.streamingAssetsPath + "/" + tSceneName + "/config";
            #elif UNITY_ANDROID
			if("scene9".Equals(tSceneName)){
				tSceneName = "fangtai";
			}
			_algPath = "/storage/emulated/0/InsightAR/" + tSceneName + "/config";
            #endif
        }
